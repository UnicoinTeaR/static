...
TimelinePlotter.prototype.Draw = function(context) {
    ...
    var timeInterval = ds.getDataAt(ds._dataItems.length-1).date - ds.getDataAt(ds._dataItems.length-2).date;
...
//    chart_switch_language('zh-cn');
    ...
    GLOBAL_VAR.requestParam = "symbol=win_btc&step=900";
...
var RequestData = function(showLoading) {
    ...
    MixedRequestData(globalurl);
};

function MixedRequestData(globalurl){
    WebsocketRequestData(GLOBAL_VAR,RequestDataSuccess);
}


function WebsocketRequestData(param,fn){
    console.info('fullkline.js 使用websocket方式,需要重写 function WebsocketRequestData');
}
function RequestDataSuccess(res) {
    var array = res.data;
