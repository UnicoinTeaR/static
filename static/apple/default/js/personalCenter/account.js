            ...
            success: function (data) {
                if (data.success) {
                    ...
                    $.each(data.data.wallets, function (index, item) {
                        ...
                        option_dom.push('<img src="' + item.coinImgUrl + '" class="market-icon">');
                        option_dom.push('<b>');
                        ...
                        option_dom.push('<span class="allworth" title=" '+language["apple.dom.msg31"]+' ￥' + common.retainDemical(item.totalCNY + item.ffrozenCNY + item.flockCNY, 4) + '">');
                        ...
                        /*总资产占比*/
                        option_dom.push('<div class="td proportion">');
                        if (total == 0) {
                            option_dom.push('<div class="cell">');
                        ...
                        if (pcenter.allfinance.cny <= 0 || isNaN(pcenter.allfinance.cny)) {
                            ...
                            var propertion = (item.totalCNY + item.ffrozenCNY + item.flockCNY) * 100 / pcenter.allfinance.cny;
                            option_dom.push(common.retainDemical(propertion, 4));
                        }
                        option_dom.push('</div>');
                        option_dom.push('</div>');
                        ...
                        option_dom.push('<div class="cell">');
                        option_dom.push('<span class="totalworth" title=" '+language["apple.dom.msg31"]+' ￥' + common.retainDemical(item.totalCNY,4) + '">');
                        ...
                        option_dom.push('<div class="cell">');
                        option_dom.push('<span class="frozenworth" title=" '+language["apple.dom.msg31"]+' ￥' + common.retainDemical(item.ffrozenCNY,4) + '">');
                    ...
                    $(".option-tbody").html(option_dom.join(""))
                }
            }
        })
    },
    ...
    openAddressModal: function (symbol) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (data) {
                    var user = data.fuser;
                    var isCny=$("input[name=isCny]").val()=="1";
                    if(isCny){
                        if(data.fbankinfoWithdraws){
                            $.each(data.fbankinfoWithdraws, function (index, item) {
                                item.fremark=item.faddress;
                                if(item.fbanknumber){
                                    item.fbanknumber=item.fbanknumber.substring(item.fbanknumber.length-4);
                                }
                                $("#cnyAddressTemplate").tpl(item).appendTo($(".address-box"));
                            })
                        }
                    }else{
                        if (data.addressWithdraws) {
                            ...
                            $.each(data, function (index, item) {
                                $("#addressTemplate").tpl(item).appendTo($(".address-box"));
                            })
                        }
                    }
                }
    ...
    detelBankAddress : function(fid) {
        util.showerrortips("", "确定删除？", {
            okbtn : function() {
                ...
                var url = "/user/del_bankinfo.html?random=" + Math.round(Math.random() * 100);
                var param = {
                    fid : fid
                };
                $.post(url, param, function(result) {
...
$(function(){
    ...
    $("#sendSMSaddress").on("click", function() {
        var phone = $("#ftelephone").val();
        msg.sendMsgCode(8, $("#fareacode").val(), phone,$("#editphone-imgcode").val());
    });
    ...
    $("#sendMailaddress").on("click", function() {
        ...
        email.sendcodemy(8, "errorTip", this.id, address);
    });
    //新增地址
    $("#virtualAddressAddButton").on("click", function() {
    ...
    $("#withdrawCnyAddrBtn").click(pcenter.addCnyAddress);
    pcenter.openAddressModal($("#symbol").val()*1);
