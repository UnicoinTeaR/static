...
pcenter = {
            ...
            success: function (data) {
                if (data.success) {
                    ...
                    $.each(data.data.wallets, function (index, item) {
                        ...
                        dom.push('<a class="btn btn-primary btn-sm" href="/n/recharge.html?coinid=' + item.fcoinid + '" id="Deposit' + index + '" >');
                        dom.push(language["apple.dom.msg32"]);
                        dom.push('</a>');
                        dom.push('<a class="" data-toggle="modal" href="/n/withdrawals.html?coinid=' + item.fcoinid + '" id="Withdrawal' + index + '" >');
                        ...
                        dom.push('<a class="" data-toggle="modal" href="/n/bill.html?coinid=' + item.fcoinid + '" id="Bills' + index + '" >');
                        dom.push(language["apple.dom.msg34"]);
                        dom.push('</a>');
                        dom.push('<a class="" data-toggle="modal" href="/n/KYCAddress.html?coinid=' + item.fcoinid + '"  id="WithdrawalAddress' + index + '" >');
    ...
    getRecord: function (type, symbol, page) {
        ...
        param.symbol = symbol;
        $('#loading').show();
        if($("#isCny").val() == "1" && type == 1){
        	var url = "/financial/deposit/cny_record.html";
            $.ajax({
                url: url,
                type: "post",
                dataType: "json",
                ...
                success: function (data) {
                	var list = data.data;
                    var dom = [];
                    dom.push('<table class="table table-bordered" id="qcrecharge"><thead><tr><th>'+language["apple.dom.msg36"]+'</th><th>'+language["apple.dom.msg85"]+'</th><th>'+language["apple.dom.msg87"]+'</th><th>'+language["apple.dom.msg40"]+'</th></tr></thead><tbody>');
                    $.each(list.data, function (index, item) {
                        ...
                        dom.push('<tr><td>' + item.fcreatetime + '</td><td>银行卡汇款</td><td>' + item.famount + '</td><td>' + fstatus + '</td></tr>');
                    })
                    dom.push('</tbody></table>');
	        ...
	        $.ajax({
	            ...
	            success: function (data) {
	                ...
	                if (list && list.data) {
	                    ...
	                    if($("#isCny").val() == "1"){
		                    if (type == 2) {
		                        dom.push('<table id="qcwithdrawals" class="table table-bordered"><thead><tr><th>'+language["apple.dom.msg59"]+'</th><th>'+language["apple.dom.msg36"]+'</th><th>'+language["apple.dom.msg81"]+'</th><th>'+language["apple.dom.msg82"]+'</th><th>'+language["apple.dom.msg39"]+'</th><th>'+language["apple.dom.msg83"]+'</th>'+'</th><th>'+language["apple.dom.msg40"]+'</th></tr></thead><tbody>');
		                    }
		                    $.each(data, function (index, item) {
		                        dom.push('<tr>');
		                        if (type == 2)
		                    ...
		                    if (type == 1) {
		                        dom.push('<table id="billcrecharge" class="table table-bordered"><thead><tr><th>'+language["apple.dom.msg59"]+'</th><th>'+language["apple.dom.msg36"]+'</th><th>'+language["apple.dom.msg37"]+'</th><th>'+language["apple.dom.msg60"]+'</th><th>'+language["apple.dom.msg53"]+'</th><th>'+language["apple.dom.msg40"]+'</th></tr></thead><tbody>');
		                    }
		                    if (type == 2) {
		                        dom.push('<table id="billwithdrawals" class="table table-bordered"><thead><tr><th>'+language["apple.dom.msg59"]+'</th><th>'+language["apple.dom.msg36"]+'</th><th>'+language["apple.dom.msg37"]+'</th><th>'+language["apple.dom.msg38"]+'</th><th>'+language["apple.dom.msg41"]+'</th><th>'+language["apple.dom.msg39"]+'</th><th>'+language["apple.dom.msg40"]+'</th></tr></thead><tbody>');
		                    }
		                    $.each(data, function (index, item) {
		                        dom.push('<tr>');
		                        if (type == 1)
		                        ...
		                        if (type == 2)
		                            dom.push('<td>' + item.fcoinname + '</td><td>' + item.fcreatetime + '</td><td>' + item.famount + '</td><td>' + item.fwithdrawaddress + '</td><td>'+ item.addressremark + '</td><td>' + _util.addNum(item.ffees,item.fbtcfees) + '</td><td>' + item.fstatus_s + '</td></tr>');
		                    })
	                    }
	                    dom.push('</tbody></table>');
	                }
    ...
    updateCnyRecharge : function(fid) {

        var url = "/deposit/selectOrder" ;
        var param = {
            desc : fid
        ...
        $.post(url, param, function(result) {
            if (result != null) {
                if (result.code == 200) {
                    $("#fownerName").html(result.data.fownerName);
                    $("#fbankNumber").html(result.data.fbankNumber);
                    $("#fbankAddress").html(result.data.fbankAddress);
                    $("#bankMoney").html(result.data.money);
                    $("#bankInfo").html(result.data.remarkNo);
                    // $("#desc").val(result.data.tradeId);
                    // $("#bankInfotips").html(result.data.remarkNo);
                    // $("#rechage1").hide();
                    // $("#rechage4").show();
                    $("#modal-schkd").modal("show");
                } else {
                    sweetAlert('', result.msg, 'error');
                }
            }
    ...
    cancelCNYRecharge : function(id) {
        util.showconfirm("确定取消该笔充值订单吗？", {
            okbtn : function() {
                $('#confirmTips').modal('hide');
                var url = "/deposit/cny_cancel.html?random=" + Math.round(Math.random() * 100);
                var param = {
                    id : id
                };
                $.post(url, param, function(result) {
...
$(function(){
    ...
    pcenter.getRecord(1,$("#symbol").val()*1,1);
    
    //人民币充值列表时间
    $("#homerp2").on("click",".completioninfo", function() {
        var fid = $(this).data().fid;
        pcenter.updateCnyRecharge(fid);
    });
    $("#homerp2").on("click",".rechargecancel", function() {
        var fid = $(this).data().fid;
        pcenter.cancelCNYRecharge(fid);
    });
})
