...
pcenter = {
    ...
    //保存cny提交
    saveCnyRecharge : function(that) {
        var payee = $("#fromPayee").val();
        var phone = $("#fromPhone").val();
        var sbank = $("#withdrawBlank").val();
        var type = $("#finType").val();
        var random = $("#random").val();
        var minMoney = $("#minRecharge").val();
        var money = 0;
        if (sbank == "") {
            sweetAlert('', language["comm.error.tips.83"], 'error');
            return;
        }
        money = $("#diyMoney").val();
        ...
        if (Number(money) < Number(minMoney) || isNaN(money)) {
            sweetAlert('', language["comm.error.tips.84"].format(minMoney), 'error');
            return;
        }
        var url = "/deposit/alipay_manual.html?random=" + $("#random").val() * 100;
        var param = {
            payee:payee,
            phone:phone,
            money : money,
        ...
        $.post(url, param, function(result) {
            ...
            if (result != null) {
                if (result.code ==200) {
                    ...
                    // $("#bankInfo").val(result.data.remarkNo);
...
//			dist : dist,
    ...
    /**
     * 页面生成人民币充值随机尾数，重新加载页面
     * @returns {string}
     */
    rechargeRandom:function() {
    // return location.reload();
        var random =  Math.random().toFixed(2)+"";
        $("#random").val(random);
    // $(".randomtips").html(random.substring(1));
    },
    ...
    sendSMSSaveAddress:function () {
        ...
        var phone = $("#ftelephoneaddress").val();
        msg.sendMsgCode(10, $("#fareacodeaddress").val(),phone,$("#editphone-imgcode").val());
    },
            ...
            success: function (res) {
                ...
                if (data) {
                    ...
                    if (data.rechargeList) {
                        ...
                        dom.push('<table id="nqcrecharge" class="table table-bordered"><thead><tr><th>'+language["apple.dom.msg36"]+'</th><th>'+language["apple.dom.msg37"]+'</th><th>'+language["apple.dom.msg52"]+'</th><th>'+language["apple.dom.msg53"]+'</th><th>'+language["apple.dom.msg40"]+'</th></tr></thead><tbody>');
                        $.each(data, function (index, item) {
                            dom.push('<tr>');
    ...
    openCnyRechargeModal: function (currentPage) {
        if(!currentPage){
            currentPage = 1;
        }
        var type = $("#finType").val();
        ...
        $(".coinRechargeName").text($("input[name=cnyName]").val());
        $('.rechargecoin').text($("input[name=cnyName]").val());
    ...
    refRecord : function() {
        if(_data.isCny){
            ...
            pcenter.openRechargeModal($("#symbol").val()*1,1);
        }
    },
...
$(function(){
    ...
    $("#withdrawCnyAddrBtn").on("click",function () {
        window.setTimeout(function(){pcenter.addCnyAddress();},Math.floor(Math.random()*300));
    });

    //绑定获取短信验证码按钮事件(添加验证码)
    $("#bindsendmessage").on("click", function() {
        pcenter.sendSMSSaveAddress()
    });

    //提交提币地址
    $("#rechargebtn").on("click", function() {
        pcenter.saveCnyRecharge()
    });

    pcenter.rechargeRandom();
    ...
    // 人民币充值列表时间
    $("#deposit1").on("click",".completioninfo", function() {
        var fid = $(this).data().fid;
        pcenter.updateCnyRecharge(fid);
    });
    $("#deposit1").on("click",".rechargecancel", function() {
        var fid = $(this).data().fid;
        pcenter.cancelCNYRecharge(fid);
    });
    //TODO 目前没有稳定币，所以都需要生成虚拟币
    // if(_data.isCny){
    //     pcenter.openCnyRechargeModal();
    // }else{
    //     pcenter.openRechargeModal($("#symbol").val()*1,1);
