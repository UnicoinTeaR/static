...
var trade = {
    ...
    createMarketDom: function (data) {
        ...
        $.each(data.data, function (index, item) {
            ...
            dom.push('<a href="/n/trade.html?symbol=' + item.code + '">' + item.name + '</a>');
            dom.push('</li>');
...
//        dom.push('</span>');
    ...
    depthchange: function () {
        ...
        //2秒钟跑一次
        setTimeout(trade.depthchange, 200000);
    },
    /**
     * 处理深度和最新成交数据
     */
...
//        /*清空卖的dish-hands*/
    ...
    handleLastKlineData: function (data) {
        ...
        $("#canbuy").text(language["apple.dom.msg69"] + "：" + " " + trade.buysell_obj.canbuy + " " + trade.leftcur);
        $("#cansell").text(language["apple.dom.msg70"] + "：" + " " + trade.buysell_obj.cansell + " " + trade.rightcur);
        ...
        $("#p_height").text(_util.numFormat(item.height, 8)).addClass('green');
        $("#p_low").text(_util.numFormat(item.low, 8)).addClass('ryhot');
        $("#p_vol").text(item.vol + " " + $("#rightCoinName").val());


        trade.canbuyuse = _util.numFormat(item.rightotal, 8);
        trade.canselluse = _util.numFormat(item.lefttotal, 8);
        ...
        trade.initScroll('buy-slider', $("#buy-volume").text(), 0, trade.canbuyuse, 0.001, trade.buyscrollcb);
        trade.initScroll('sell-slider', $("#sell-amount").val(), 0, trade.canselluse, 0.001, trade.sellscrollcb);
        //修改页面title属性
        if (item.p_new) {
    ...
    buyscrollcb: function (value) {
        ...
        if ($("#buy-price").val() * 0.001 > trade.canbuyuse) {
            $("#buy-volume").text(0);
            $("#buy-amount").val(0);
        } else {
    ...
    sellscrollchange: function (amount) {
        $('#sell-slider').find('.ui-slider-handle').css('left', amount.val() * 100 / trade.canselluse + "%");
        $('#sell-slider').find('.ui-slider-range').css("width", amount.val() * 100 / trade.canselluse + "%")
    },
    buyinput: function () {
        var price = $("#buy-price");
        ...
        if (price.val() && amount.val()) {
            ...
            if (volume.text() * 1 < trade.canbuyuse * 1) {
                trade.buyscrollchange(volume);
            } else {
                amount.val(_util.numFormat(trade.canbuyuse / price.val(), 3));
    ...
    buyamountinput: function () {
        ...
        /*如果与价格框相乘大于了可用*/
        if (price.val() * amount.val() > trade.canbuyuse) {
            amount.val(_util.numFormat(trade.canbuyuse / price.val(), 3));
            volume.text(_util.numFormat(amount.val() * price.val(), 8));
            trade.buyscrollchange(volume);
        } else {
    ...
    getEntrustData: function () {
        ...
        //2秒钟跑一次，获取委单数据信息
        setTimeout(trade.getEntrustData, 2000000);
    },
    /**
     * 处理委单数据
     * @param data
